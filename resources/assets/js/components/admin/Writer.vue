<template>
	...
	<div class="form-group">
	  	<select required v-for="crew in crewCount" class="form-control" name="writer[]" >
		  	<option></option>
		    <option v-for="producer in producers" :key="producer.id" :value=setValue(producer.id,producer.name)>{{ producer.name }}</option>
	  	</select>
		<a href="javascript:void(0)" @click="addCrew" type="button" id="addrow">Add Film Writer</a>
	</div>
